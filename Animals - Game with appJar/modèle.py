...
authorized_lieux = ('litière', 'mangeoire', 'nid')
...
def change_lieu(animal_id, LIEU):
    with open('animal.json', "r", encoding="utf-8") as file:
        animal = json.load(file)
        with open('equipment.json', "r", encoding="utf-8") as file2:
            equipment = json.load(file2)
            if animal_id not in animal:
                print(f"Désolé, {animal_id} n'est pas un animal connu")
                file.close()
                return None
            ...
            for i in animal:
                for x in equipment:                   
                    if (LIEU == x and equipment[LIEU]["DISPONIBILITÉ"] == "occupé"):
                        print(f'Désolé, le lieu {LIEU} est déjà occupé')
                        return None

                    if ((LIEU == x and equipment[LIEU]["DISPONIBILITÉ"] == "libre") and (i == animal_id)):
                        ancien_lieu = animal[i]["LIEU"]
                        equipment[ancien_lieu]["DISPONIBILITÉ"] = "libre"
                        if equipment[LIEU] != "litière":
                            equipment[LIEU]["DISPONIBILITÉ"] = "occupé"
                        animal[i]["LIEU"] = LIEU
                        json.dump(equipment, open("equipment.json", "w", encoding="utf-8"), indent=4)
                        json.dump(animal, open("animal.json", "w", encoding="utf-8"), indent=4)
                        return None                
